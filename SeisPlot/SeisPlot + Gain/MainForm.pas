...
uses
  ...
  Controls, TeEngine, Series, Math, Graphics, TeeStore,
  TeeProcs, Chart, StdCtrls, SysUtils, Forms, ToolWin, TeeTools;
...
type
  TFirstForm = class(TForm)
    ...
    procedure SaveChart1Click(Sender: TObject);
...
//      Number_of_Lines:=2;
      ...
      for i:=0 to Number_of_Blocks-1 do
        begin
          ...
          S1:=TrimLeft(S1);
          S1:=TrimRight(S1);
          SetLength(Start_Point, i+1);
          Start_Point[i]:=StrToFloat(S1);
          Horizontal_Value:=Start_Point[i];
          S1:=Copy(S,11,16);
          S1:=TrimLeft(S1);
          S1:=TrimRight(S1);
          ...
          Filling_Area1[i].YOrigin:=Start_Point[i];
          ...
          Filling_Area1[i].AddXY(Start_Point[i],0);
                      ...
                      if Horizontal_Value>0 then
                        begin
                              if Previous_Horizontal_Value<=0 then
                                begin
                                  ...
                                  Filling_Area1[i].AddXY(Start_Point[i], Residual,'',clLime);
                                end;
                              Filling_Area1[i].AddXY(Start_Point[i] + Horizontal_Value, Vertical_Value,Label_String,clLime);
                        end
                        ...
                        begin
                              if Previous_Horizontal_Value>=0 then
                                begin
                                  Residual:= Vertical_Value - ((Vertical_Value-Previous_Vertical_Value)/(Horizontal_Value-Previous_Horizontal_Value) * Horizontal_Value);
                                  Filling_Area1[i].AddXY(Start_Point[i], Residual,'',clBlue);
                                end;
                              Filling_Area1[i].AddXY(Start_Point[i] + Horizontal_Value, Vertical_Value ,Label_String,clBlue);
                        end
                      else
                        begin
                          if Previous_Horizontal_Value>0 then
                            Filling_Area1[i].AddXY(Start_Point[i], Vertical_Value,Label_String,clBlue)
                          else if Previous_Horizontal_Value<0 then
                            Filling_Area1[i].AddXY(Start_Point[i], Vertical_Value,Label_String,clLime)
                          else
                            Filling_Area1[i].AddXY(Start_Point[i], Vertical_Value,Label_String,clBlue)
                        end;
                      Series[i].AddXY(Start_Point[i] + Horizontal_Value,Vertical_Value, Label_String);
                   end;
                  Previous_Vertical_Value:=Vertical_Value;
                  Previous_Horizontal_Value:=Horizontal_Value;
          ...
          Filling_Area1[i].AddXY(Start_Point[i], End_Time);
          Series[i].AddXY(Start_Point[i], End_Time);
        end;
      Lines.Free;
...
//      Number_of_Lines:=2;
      ...
      for i:=0 to Number_of_Blocks-1 do
        begin
          ...
          Filling_Area2[i].YOrigin:=Start_Point[i];
          ...
          Filling_Area2[i].AddXY(0,Start_Point[i]);
          SetLength(Series, Length(Series)+1);
          ...
          Series[i].Color:=clBlack;
          Series[i].Stairs:=False;
          Series[i].AddXY(0,Start_Point[i]);
          Begining_of_Block:=Row_Number;
          for j:=1 to Number_of_Rows do
            begin
              Row_Number:=Row_Number+1;
                      ...
                      if Vertical_Value>0 then
                        begin
                              if Previous_Vertical_Value<=0 then
                                begin
                                  ...
                                  Filling_Area2[i].AddXY(Residual, Start_Point[i],Label_String,clBlue);
                                end;
                              Filling_Area2[i].AddXY(Horizontal_Value, Start_Point[i] + Vertical_Value,'',clLime);
                        end
                        ...
                        begin
                              if Previous_Vertical_Value>=0 then
                                begin
                                  Residual:= Horizontal_Value - ((Horizontal_Value-Previous_Horizontal_Value)/(Vertical_Value-Previous_Vertical_Value) * Vertical_Value);
                                  Filling_Area2[i].AddXY(Residual, Start_Point[i],Label_String,clLime);
                                end;
                              Filling_Area2[i].AddXY(Horizontal_Value, Start_Point[i] + Vertical_Value,Label_String,clBlue);
                        end
                      else
                        begin
                          if Previous_Vertical_Value<0 then
                            Filling_Area2[i].AddXY(Horizontal_Value, Start_Point[i],Label_String,clBlue)
                          else if Previous_Vertical_Value>0 then
                            Filling_Area2[i].AddXY(Horizontal_Value, Start_Point[i],Label_String,clLime)
                          else
                            Filling_Area2[i].AddXY(Horizontal_Value, Start_Point[i],Label_String,clLime)
                        end;
                      Series[i].AddXY(Horizontal_Value, Start_Point[i]+Vertical_Value, Label_String);
                   end;
                  Previous_Vertical_Value:=Vertical_Value;
                  Previous_Horizontal_Value:=Horizontal_Value;
          ...
          Filling_Area2[i].AddXY(End_Time, Start_Point[i]);
          Series[i].AddXY(End_Time,Start_Point[i]);
        end;
      Lines.Free;
      Options1.Checked:=True;
      Screen.Cursor:=crDefault;
...
procedure TFirstForm.Increase1Click(Sender: TObject);
var
i,j: integer;
begin
  for i:=0 to Length(Series)-1 do
      ...
      if Length(Filling_Area1)>0 then
        begin
          for j:=0 to Series[i].Count-1 do
            begin
              Series[i].XValue[j]:=Start_Point[i]+(Series[i].XValue[j]-Start_Point[i])*1.25;
            end;
          for j:=0 to Filling_Area1[i].Count-1 do
            begin
              Filling_Area1[i].XValue[j]:=Start_Point[i]+(Filling_Area1[i].XValue[j]-Start_Point[i])*1.25;
            end;
        end;
      if Length(Filling_Area2)>0 then
        begin
          for j:=0 to Series[i].Count-1 do
            begin
              Series[i].YValue[j]:=Start_Point[i]+(Series[i].YValue[j]-Start_Point[i])*1.25;
            end;
          for j:=0 to Filling_Area2[i].Count-1 do
            begin
              Filling_Area2[i].YValue[j]:=Start_Point[i]+(Filling_Area2[i].YValue[j]-Start_Point[i])*1.25;
            end;
        end;
    end;
end;

procedure TFirstForm.Decrease1Click(Sender: TObject);
var
i,j: integer;
begin
  for i:=0 to Length(Series)-1 do
...
procedure TFirstForm.SaveChart1Click(Sender: TObject);
begin
  if SaveDialog2.Execute then
    SaveChartToFile(TCustomChart(Chart1), SaveDialog2.FileName, False, False);
end;

procedure TFirstForm.LoadChart1Click(Sender: TObject);
begin
  if OpenDialog2.Execute then
    LoadChartFromFile(TCustomChart(Chart1), OpenDialog2.FileName);
end;
