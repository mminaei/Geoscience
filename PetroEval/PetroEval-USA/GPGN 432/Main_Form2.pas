...
uses
  ...
  TMainForm2 = class(TForm)
    ...
    DBChart10: TDBChart;
    HorizLineSeries7: THorizLineSeries;
    HorizLineSeries8: THorizLineSeries;
    HorizLineSeries9: THorizLineSeries;
...
//    procedure ChangePath;
    ...
    procedure DBChart9Scroll(Sender: TObject);
    procedure DBChart9UndoZoom(Sender: TObject);
    procedure Water1Click(Sender: TObject);
    procedure DBChart10DblClick(Sender: TObject);
...
uses ZoomedChart, PreviewForm, FirstForm,  LoadForm, PorosityForm, SwForm;
...
(* to find track unit  *)
    ...
    Properties.Fields[3].Value:=-0.15;
    Properties.Append;
    Properties.Fields[0].Value:='Sg';
    Properties.Fields[1].Value:='%';
    Properties.Fields[2].Value:=100;
    Properties.Fields[3].Value:=0;
    Properties.Append;
    Properties.Fields[0].Value:='DPHI';
    Properties.Fields[1].Value:='v/v_dec';
    Properties.Fields[2].Value:=0.45;
    Properties.Fields[3].Value:=-0.15;
    ...
    Values.FieldDefs.Add('Sg', ftFloat, 0, False);
    Values.FieldDefs.Add('DPHI', ftFloat, 0, False);
...
procedure TMainForm2.Find_DPHI;
var
i:integer;
Temp_Name: string;
begin
  with OpenedFile do
    begin
      ...
      DPHI_Track_Number:=-1;
      for i:=0 to NoTracks+8 do
        begin
          Temp_Name:= Properties.Fields[0].AsString;
          if (Temp_Name='DPHI') then
            begin
...
begin
  with OpenedFile do
    begin
      Properties.First;
      DBChart1.Series[4].YValues.ValueSource:=Values.FieldDefs[0].Name;
      DBChart1.Series[4].YValues.Name:=Values.FieldDefs[0].Name;
      DBChart1.Series[4].DataSource:=Values;
      Sg_Track_Number:=-1;
      for i:=0 to NoTracks+5 do
        begin
          Temp_Name:= Properties.Fields[0].AsString;
          if (Temp_Name='Sg') then
            begin
              DBChart1.Series[4].XValues.ValueSource:=Values.FieldDefs[i].Name;
              DBChart1.Series[4].XValues.Name:=Values.FieldDefs[i].Name;
              DBChart5.CustomAxes[4].Title.Caption :=Properties.FieldByName('TRACK_NAME').AsString + ' ('+ Properties.FieldByName('TRACK_UNIT').AsString + ')';
              Sg_Track_Number:=i;
              Break;
            end;
          Properties.Next;
...
begin
  with OpenedFile do
    begin
      Properties.First;
      DBChart9.Series[0].YValues.ValueSource:=Values.FieldDefs[0].Name;
      DBChart9.Series[0].YValues.Name:=Values.FieldDefs[0].Name;
      DBChart9.Series[0].DataSource:=Values;
      MSFL_Track_Number:=-1;
      for i:=0 to NoTracks-1 do
        begin
          Temp_Name:= Properties.Fields[0].AsString;
          if ((Temp_Name='MSFL')or(Temp_Name='SFLA')or(Temp_Name='SFLU')or(Temp_Name='AHO10')or(Temp_Name='LL8')) then
            begin
              DBChart9.Series[0].XValues.ValueSource:=Values.FieldDefs[i].Name;
              DBChart9.Series[0].XValues.Name:=Values.FieldDefs[i].Name;
...
begin
  with OpenedFile do
    begin
      Properties.First;
      DBChart9.Series[1].YValues.ValueSource:=Values.FieldDefs[0].Name;
      DBChart9.Series[1].YValues.Name:=Values.FieldDefs[0].Name;
      DBChart9.Series[1].DataSource:=Values;
      LLM_Track_Number:=-1;
      for i:=0 to NoTracks-1 do
        begin
          Temp_Name:= Properties.Fields[0].AsString;
          if ((Temp_Name='LLS')or(Temp_Name='LLM')or(Temp_Name='ILS')or(Temp_Name='ILM')or(Temp_Name='HLLS')or(Temp_Name='AHT30')or(Temp_Name='AHT20')or(Temp_Name='AHT60')) then
            begin
              DBChart9.Series[1].XValues.ValueSource:=Values.FieldDefs[i].Name;
              DBChart9.Series[1].XValues.Name:=Values.FieldDefs[i].Name;
...
begin
  with OpenedFile do
    begin
      Properties.First;
      DBChart9.Series[2].YValues.ValueSource:=Values.FieldDefs[0].Name;
      DBChart9.Series[2].YValues.Name:=Values.FieldDefs[0].Name;
      DBChart9.Series[2].DataSource:=Values;
      LLD_Track_Number:=-1;
      for i:=0 to NoTracks-1 do
        begin
          Temp_Name:= Properties.Fields[0].AsString;
          if ((Temp_Name='LLD')or(Temp_Name='ILD')or(Temp_Name='HLLD')or(Temp_Name='AHF90')or(Temp_Name='AHF60')or(Temp_Name='AHT90')) then
            begin
              DBChart9.Series[2].XValues.ValueSource:=Values.FieldDefs[i].Name;
              DBChart9.Series[2].XValues.Name:=Values.FieldDefs[i].Name;
...
procedure TMainForm2.DBChart9Scroll(Sender: TObject);
var
i:integer;
begin
  for i:=0 to 2 do
    begin
      DBChart10.CustomAxes[i].Minimum:=DBChart9.CustomAxes[i].Minimum;
      DBChart10.CustomAxes[i].Maximum:=DBChart9.CustomAxes[i].Maximum;
    end;
end;

procedure TMainForm2.DBChart9UndoZoom(Sender: TObject);
var
i:integer;
begin
  for i:=0 to 2 do
...
procedure TMainForm2.Water1Click(Sender: TObject);
var
No_Values: Integer;
Rw_Value: Single;
Rt_Value: Single;
...
with OpenedFile do
  ...
  for i:=0 to No_Values-1 do
    begin
      ...
      Depth_Value:=Values.Fields[0].Value;
      if not Value_Is_Constant[1] then
        Rw_Value:=Values.Fields[Rw_Track_Number].Value
      else
        Rw_Value:=Constant_Value[1];
      if not Value_Is_Constant[2] then
        Rt_Value:=Values.Fields[Rt_Track_Number].Value
      else
        Rt_Value:=Constant_Value[2];
      if not Value_Is_Constant[3] then
        PHI_Value:=Values.Fields[PHI_Track_Number].Value
      else
        PHI_Value:=Constant_Value[3];

      if ((PHI_Value<> OpenedFile.NullValue)and(Rt_Value<> OpenedFile.NullValue)and(Rw_Value<> OpenedFile.NullValue)) then
        begin
          if PHI_Value<0 then PHI_Value:=0;
          if PHI_Value>1 then PHI_Value:=1;
          if PHI_Value=0 then PHI_Value:=0.000000001;
          if Rt_Value=0 then Rt_Value:=0.000000001;

          Fr_Value:= a_Value/Power(PHI_Value, m_Value);
          Sw_Value:=Power(Fr_Value * Rw_Value / Rt_Value, 1/Saturation_Exponent);
          Sw_Value:=Sw_Value * 100;
          if Sw_Value<0 then Sw_Value:=0;
          if Sw_Value>100 then Sw_Value:=100;

          Sg_Value:= 100 - Sw_Value;
          Values.FieldByName('Sg').Value:=Sg_Value;
        end
      else
        begin
          Values.Fields[Values.FieldDefs.Count-1].Value:=OpenedFile.NullValue;
...
procedure TMainForm2.DBChart10DblClick(Sender: TObject);
begin
  ChartEditor1.Chart:=DBChart10;
  ChartEditor1.Execute;
  CheckAxes;
end;
