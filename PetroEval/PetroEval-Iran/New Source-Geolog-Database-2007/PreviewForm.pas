...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls,  ClipBrd, Printers, Buttons;
...
uses Main_Form, WaitingForm, Main_Form2;
...
//    //GmPreview1.Canvas.TextOut(10,50, Label2.Caption);
...
//    //GmPreview1.Canvas.Font:=VrScrollText1.Font;
//    //GmPreview1.Canvas.TextOut((//GmPreview1.PageWidth[//GmPixels] - VrScrollText1.Width)/2,50, 'Petroleum University of Technology');
...
//  AvailableHeight:=Round(//GmPreview1.PageHeight[//GmPixels])- FooterHeight - HeaderHeight;
  if Print_From=0 then
    begin
      with MainForm do
        begin
          if Charts[0].Height > AvailableHeight then
            begin
              for i:=1 to (Charts[0].Height div AvailableHeight) do
                begin
                  //GmPreview1.NewPage;
                  PrintArms;
                end;
              ChartPages:=Charts[0].Height div AvailableHeight + 1;
              ChartHeightPerPage:=AvailableHeight;
              for j:=0 to Length(Charts)-1 do
                begin
                  if Charts[j].Visible=True then
                    begin
                      Charts[j].CopyToClipboardBitmap;
                      try
                        ...
                        for i:=0 to ChartPages-1 do
                          begin
                            R1:=Rect(0, i*ChartHeightPerPage, Charts[j].Width, (i+1)*ChartHeightPerPage);
                            R2:=Rect(0, 0, Charts[j].Width, ChartHeightPerPage);
                            try
                              Bitmap2:=TBitmap.Create;
                              Bitmap2.Width:=Bitmap.Width;
                              Bitmap2.Height:=ChartHeightPerPage;
                              Bitmap2.Canvas.CopyRect(R2, Bitmap.Canvas, R1);
              ...
              for i:=0 to Length(Charts)-1 do
                begin
                  if Charts[i].Visible=True then
                    begin
                      j:=Charts[i].Width;
                      Charts[i].CopyToClipboardBitmap;
                      Bitmap:=TBitMap.Create;
                      try
                        Bitmap.LoadFromClipboardFormat(cf_BitMap,ClipBoard.GetAsHandle(cf_Bitmap),0);
                        //GmPreview1.Canvas.Draw( k ,HeaderHeight,Bitmap);
                        k:=k+j;
                      finally
                        Bitmap.Free;
                      end;
...
//                    R2:=Rect(0, 0, DBChart1.Width, ChartHeightPerPage);
                    try
                      R1:=Rect(0, i*ChartHeightPerPage, DBChart1.Width, (i+1)*ChartHeightPerPage);
...
begin
  if Print_From=0 then
    begin
      MainForm.StatusBar1.Panels[0].Text:='Please wait...';
      MainForm.StatusBar1.Refresh;
      Waiting_Form.Show;
